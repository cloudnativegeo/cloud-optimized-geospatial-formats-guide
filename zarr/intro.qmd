...
::: {.callout-note}
Zarr data chunks do not necessarily need to be stored in the same storage system as the Zarr metadata. This is what enables [virtual Zarr stores](./virtual-zarr.qmd) where the metadata references data in legacy chunked data formats (such as NetCDF and HDF5).
:::

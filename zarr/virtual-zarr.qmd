...
Virtual Zarr lets you interact with chunked archival data as if it were Zarr. You can get the benefits of Zarr without ever duplicating the data. 

Virtual Zarr stores capture metadata about where particular chunks of data are stored and allows tools like xarray to access the data from the chunks directly. These virtual Zarr stores can be written to disk using Kerchunk or Icechunk and distributed alongside data.

## How does it relate to regular Zarr?

Zarr can be separated into two separate specifications. There is the Zarr interface which describes how libraries such as xarray interact with Zarr Arrays and Stores. That Zarr interface is implemented in libraries such as zarr-python, zarrs, and zarrita.js. Then there is the storage specification. When we think of Zarr stores we normally think of "native" Zarr Stores where the data is stored alongside the metadata. They are in the same hierarchy and sit on the same bucket on cloud storage.
...
::: {.callout-warning}
A major limitation of virtual Zarr stores is that the chunking of data will always be constrained to the chunk structure of the underlying data format. With virtualization you can never change the chunks or the compression of the original data.
:::
...
[`kerchunk`](https://fsspec.github.io/kerchunk) is a Python library which generates a set of references to data or URLs under a key value store that matches the Zarr spec. 

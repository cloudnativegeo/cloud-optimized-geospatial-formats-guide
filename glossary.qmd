...
Some providers set a minimum mean [object](#object-storage-cloud-storage) size for each tier, because indexing very many small objects is expensive. A layout whose objects fall below that floor can be refused or moved to a slower tier, so [chunk size](#chunk-size) and grouping choices have to be made against the policy of the storage you are actually using.

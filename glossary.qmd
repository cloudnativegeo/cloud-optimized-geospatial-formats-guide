...
What subsets of the data get requested: a time series at a single point, a spatial window at a single date, or a full scan of everything.

The access pattern drives the [chunk](#chunk) shape and the order of the dimensions, which is a separate question from [chunk size](#chunk-size). A cube chunked long in time answers a time series at one point cheaply and a full map expensively; chunked wide in latitude and longitude it does the opposite. Only rewriting the data lets you change it, so a virtualized dataset inherits whatever access pattern its source was written for.
...
The definition of shard here is specific to [Zarr](#zarr). A shard is a single object in object storage that contains multiple [chunks](#chunk) of data. See also: [Zarr sharding](https://zarr.readthedocs.io/en/latest/user-guide/performance/#sharding).
...
A storage service that keeps data on different classes of hardware at different prices and speeds. A common arrangement is a "warm" tier on disk for data that is read often, and a "cold" tier on tape or archival storage for data that is read rarely.

Some providers set a recommended minimum mean [object](#object-storage-cloud-storage) size for each tier. The storage service holds one index entry for each object, and very many small objects can fill that index before the storage capacity is full. Some providers enforce this limit. Other providers give it as guidance only. Find the policy of the service that you use, then make your [chunk size](#chunk-size) and grouping choices against it.

...
Zarr, despite it's name, is not a scary format. It's designed for data that is too bit for users' machines, but it makes that data small and organizes it in a way where users can take just the bits they need or distribute the load of processing lots of thos bits (aka chunks) across many machines.
